...
ipcMain.handle('audio:process', async(_e,p={})=>{
  try{
    ...
    const sys=`You are a professional video prompt engineer. Your output MUST be a JSON array of scene strings. ALL output text, including titles and descriptions, MUST be in ENGLISH. Each scene must strictly follow this format: Scene 01 – Title | Character1: full description | Character2: full description | Style: ... | Character voices: ... | Camera: ... | Setting: ... | Mood: ... | Audio cues: ... | Dialog: ... | Subtitles: ... . All parts of the format must be in English. Style JSON người dùng cung cấp phải được tôn trọng. Lời thoại: ${p.dialog?'có':'không'}. Phụ đề: ${p.subtitles?'có':'không'}. Yêu cầu thêm của người dùng phải được áp dụng vào mọi prompt nếu có. Compare the transcript with the original text if provided, fix the transcript to match the original content, then generate the final prompt based on the original text. Output everything in ENGLISH.`;
    const prompt=`STYLE JSON:\n${p.styleJson||''}\n\nORIGINAL TEXT:\n${p.originalText||''}\n\nEXTRA PROMPT REQUIREMENTS:\n${p.extraRequirement||''}\n\nAUDIO TRANSCRIPT:\n${raw}\n\nGenerate the final prompts based on the original content. Ensure ALL output is in ENGLISH.`;
